...
A. backend
    ...
    2- then run : poetry lock
    3- make sure you are connected to the vpn with AWS and the db is accessible
    4- generate prisma client: poetry run prisma generate
    5- update the db: poetry run prisma migrate dev (reset if you have to)
                or run all the migration sql scripts if you know where to start from
    6- to run (debug), use the debugger (backend)
